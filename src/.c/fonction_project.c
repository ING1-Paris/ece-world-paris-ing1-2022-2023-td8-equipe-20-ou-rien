...
void menuStat(int statSnake,float statFight,int statBallon,BITMAP *buffer, int fpsStat, int taupeStat, int riviereStat)
{
    char tmpString[30];
        while(!key[KEY_ESC])
        {
            ...
            textout_ex(buffer,font,"Japan apocalypse :",0,200, makecol(255,0,0),-1);
            sprintf(tmpString,"%d",fpsStat);
            textout_ex(buffer,font,tmpString,150,200, makecol(255,0,0),-1);
            textout_ex(buffer,font,"Taupe la :",0,250, makecol(255,0,0),-1);
            sprintf(tmpString,"%d",taupeStat);
            textout_ex(buffer,font,tmpString,100,250, makecol(255,0,0),-1);
            textout_ex(buffer,font,"Rivière :",0,300, makecol(255,0,0),-1);
            sprintf(tmpString,"%d", riviereStat);
            textout_ex(buffer,font,tmpString,100,300, makecol(255,0,0),-1);
            blit(buffer,screen,0,0,0,0,SCREEN_W,SCREEN_H);
        }
}
        ...
        if(joueur1->BoolTour)
        {
            if((joueur1->posX==295&&joueur1->posY==440))
            {
                ...
                if(key[KEY_ENTER])
                {
                    ...
                    destroy_bitmap(sousbuffer);
                    animationTourJoueurPourJeux(buffer,EnsembleChiffre,joueur1);
                    verifGagnant[0]=gameLoop();
                    animationTourJoueurPourJeux(buffer,EnsembleChiffre,joueur2);
                    verifGagnant[1]=gameLoop();
                    if(verifGagnant[0]>verifGagnant[1])
                    {
                        joueur1->nbTicket++;
                        joueur2->nbTicket--;
                        if(verifGagnant[0]>*fpsStat)
                        {
                            *fpsStat=verifGagnant[0];
                        }
                    }
                    else
                    {
                        joueur1->nbTicket--;
                        joueur2->nbTicket++;
                        if(verifGagnant[1]>*fpsStat)
                        {
                            *fpsStat=verifGagnant[1];
                        }
                    }
            ...
            if((joueur1->posX==225&&joueur1->posY==135))
            {
                ...
                if (key[KEY_ENTER]) {
                    ...
                    animationTourJoueurPourJeux(buffer,EnsembleChiffre,joueur2);
                    verifGagnant[1]=startGameMole();
                    if(verifGagnant[0]>verifGagnant[1])
                    {
                        joueur1->nbTicket++;
                        joueur2->nbTicket--;
                        if(verifGagnant[0]>*taupeStat)
                        {
                            *taupeStat=verifGagnant[0];
                        }
                    }
                    else
                    {
                        joueur1->nbTicket--;
                        joueur2->nbTicket++;
                        if(verifGagnant[1]>*taupeStat)
                        {
                            *taupeStat=verifGagnant[1];
                        }
                    }
                    joueur1->BoolTour=0;
                    joueur2->BoolTour=1;
...
void RiviereMap(t_joueur *joueur1,t_joueur *joueur2,BITMAP *skin1MvmtDown[5],BITMAP *skin1MvmtUp[5],BITMAP *skin1MvmtCoter[5], BITMAP *skin2MvmtDown[5],BITMAP *skin2MvmtUp[5],BITMAP *skin2MvmtCoter[5],BITMAP *skin3MvmtDown[5],BITMAP *skin3MvmtUp[5],BITMAP *skin3MvmtCoter[5],BITMAP *buffer,int frame,int *Map,int *snakeStat,float *fightStat,int *statBallon, int *fpsStat, int *taupeStat, int *riviereStat, BITMAP **EnsembleChiffre)
{
    if(!joueur1->loaded && *Map==3)
    {
        if(joueur1->BoolTour)
        {
            joueur1->posY=SCREEN_H-80;
    ...
    BITMAP *portail= importeImage("../image/image play map/snake/batiment/portailBas.bmp");
    BITMAP *sousbuffer= create_bitmap(SCREEN_W,SCREEN_H);
    SAMPLE *bgMusic = load_wav("../son/iceBgSound.wav");

    int verifGagnant[2];
    char path[50];
    checkPtrNull(bgMusic, "Exit Failure: loading ice background music failed");
    play_sample(bgMusic,200,128,1000,TRUE);
    while(1)
    {
        if(verifTicket(joueur1,joueur2,buffer))
        ...
        if(joueur1->BoolTour)
        {
            if((joueur1->posX==130&&joueur1->posY==325))
            {
                ...
                if(key[KEY_ENTER])
                {
                    stop_sample(bgMusic);
                    animationTourJoueurPourJeux(buffer,EnsembleChiffre,joueur1);
                    verifGagnant[0]=fonction_riviere();
                    animationTourJoueurPourJeux(buffer,EnsembleChiffre,joueur2);
                    verifGagnant[1]=fonction_riviere();
                    play_sample(bgMusic,200,128,1000,TRUE);
                    if(verifGagnant[0]>verifGagnant[1])
                    {
                        joueur1->nbTicket++;
                        joueur2->nbTicket--;
                        if(verifGagnant[0]>*riviereStat)
                        {
                            *riviereStat=verifGagnant[0];
                        }
                    }
                    else
                    {
                        joueur1->nbTicket--;
                        joueur2->nbTicket++;
                        if(verifGagnant[1]>*riviereStat)
                        {
                            *riviereStat=verifGagnant[1];
                        }
                    }
...
void playMap(int *BoolMenu, int *BoolSettings, int *BoolPlay, int choiceMenu)
{
    ...
    if (choiceMenu == 2) {
        ...
        loadGame(path, joueur1, joueur2,&statSnake,&statFight,&statBallon,&fpsStat,&taupeStat, &riviereStat);
        if (joueur1->BoolTour)
            Map = joueur1->Map;
        else
            Map = joueur2->Map;
    ...
    if (joueur1->skin != 1 && joueur2->skin != 1) {
        for (int i = 1; i < 5; i++) {
            destroy_bitmap(skin1MvmtDown[i]);
            destroy_bitmap(skin1MvmtUp[i]);
            destroy_bitmap(skin1MvmtCoter[i]);
        }
        free(skin1MvmtDown);
        free(skin1MvmtUp);
        free(skin1MvmtCoter);
    }
    if (joueur1->skin != 2 && joueur2->skin != 2) {
        for (int i = 1; i < 5; i++) {
            destroy_bitmap(skin2MvmtDown[i]);
            destroy_bitmap(skin2MvmtUp[i]);
            destroy_bitmap(skin2MvmtCoter[i]);
        }
        free(skin2MvmtDown);
        free(skin2MvmtUp);
        free(skin2MvmtCoter);
    }
    if (joueur1->skin != 3 && joueur2->skin != 3) {
        for (int i = 1; i < 5; i++) {
            destroy_bitmap(skin3MvmtDown[i]);
            destroy_bitmap(skin3MvmtUp[i]);
            destroy_bitmap(skin3MvmtCoter[i]);
        }
        free(skin3MvmtDown);
        free(skin3MvmtUp);
        free(skin3MvmtCoter);
    ...
    while (!key[KEY_ESC])
    {
        if(Map==0||Map==-1)
        {
            joueur1->Map=0;
            joueur2->Map=0;
            snakeMap(joueur1,joueur2,skin1MvmtDown,skin1MvmtUp,skin1MvmtCoter,skin2MvmtDown,skin2MvmtUp,skin2MvmtCoter,skin3MvmtDown,skin3MvmtUp,skin3MvmtCoter,buffer,EnsembleChiffre,frame,&Map,&statSnake,&statFight,&statBallon, &fpsStat, &taupeStat, &riviereStat);
        }
        else if(Map==1||Map==-2)
        {
            joueur1->Map=1;
            joueur2->Map=1;
            ninjaMap(joueur1,joueur2,skin1MvmtDown,skin1MvmtUp,skin1MvmtCoter,skin2MvmtDown,skin2MvmtUp,skin2MvmtCoter,skin3MvmtDown,skin3MvmtUp,skin3MvmtCoter,buffer,frame,&Map,&statSnake,&statFight,&statBallon, &fpsStat, &taupeStat, EnsembleChiffre, &riviereStat);
        }
        else if(Map==2||Map==-3)
        {
            joueur1->Map=2;
            joueur2->Map=2;
            BallonMap(joueur1,joueur2,skin1MvmtDown,skin1MvmtUp,skin1MvmtCoter,skin2MvmtDown,skin2MvmtUp,skin2MvmtCoter,skin3MvmtDown,skin3MvmtUp,skin3MvmtCoter,EnsembleChiffre,buffer,frame,&Map,&statSnake,&statFight,&statBallon, &fpsStat, &taupeStat, &riviereStat);
        }
        else if(Map==3)
        {
            joueur1->Map=3;
            joueur2->Map=3;
            RiviereMap(joueur1,joueur2,skin1MvmtDown,skin1MvmtUp,skin1MvmtCoter,skin2MvmtDown,skin2MvmtUp,skin2MvmtCoter,skin3MvmtDown,skin3MvmtUp,skin3MvmtCoter,buffer,frame,&Map,&statSnake,&statFight,&statBallon, &fpsStat, &taupeStat, &riviereStat, EnsembleChiffre);
        }
        else if(Map==4)
        {
            break;
