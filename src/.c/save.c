...
void writeSave(char path[], t_joueur *joueur1, t_joueur *joueur2)
{
...
void saveGame(char entire_path[], t_joueur *joueur1, t_joueur *joueur2) // créer le dossier de la sauvegarde voulu
{
    struct stat st = {0};
    ...
    while (!key[KEY_ESC]) {
        ...
        if (indexPath != 0 && stat(entire_path, &st) == -1) { // vérifie si la sauvegarde n'existe pas déjà
            if (key[KEY_ENTER]) {
                writeSave(entire_path, joueur1, joueur2);
                return;
            }
            alreadyExist = 0;
        } else {
